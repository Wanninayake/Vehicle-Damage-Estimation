...
# training steps till ending
HOW_MANY_TRAINING_STEPS = 1000

# learning rate
LEARNING_RATE = 0.01

# percentage of images to use as a test set
TESTING_PERCENTAGE = 10

# percentage of images to use as a validation set
VALIDATION_PERCENTAGE = 10

# evaluation interval between the training results
EVAL_STEP_INTERVAL = 10

# images size at a time
TRAIN_BATCH_SIZE = 100

# How many images to test on. This test set is only used once, to evaluate the final accuracy of the model after
# training completes.  A value of -1 causes the entire test set to be used, which leads to more stable results.
TEST_BATCH_SIZE = -1

# Image size in an evaluation batch. This validation set is used much more often than the test set, this is an indicator
# of how accurate the model is during training. A value of -1 causes the entire validation set to be used, which leads
# to more stable results across training iterations, but may be slower on large training sets.
...
def main():
    ...
    with tf.Session(graph=graph) as sess:
        ...
        if doDistortImages:
            #  setup the operations to apply distortions.
            ...
            # cache 'bottleneck' image summaries
